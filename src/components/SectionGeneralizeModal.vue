...
const props = defineProps<{
  modelValue: boolean;
  sectionLabel: string; // ex: "Formules"
...
function computeKpisFor(v: any, dureeMois: number) {
  try {
    // on force le mode BASE (pas de previews ici)
  ...
  <div v-if="isOpen" class="ov" @click.self="close">
    <div class="modal">
      ...
      <div class="foot">
        ...
        <button class="btnPrimary" @click="submit" :disabled="!allIds.length || (mode === 'SELECT' && !selectedIds.length)">
          Généraliser
        </button>
      </div>
    </div>
...
.radio{display:flex; align-items:center; gap:8px; font-weight:900; font-size:12px; color:#111827;}
.spacer{flex:1 1 auto;}
.in{
  width:min(320px, 100%);
  border:1px solid #e5e7eb; border-radius:12px;
  padding:9px 11px; outline:none; font-size:12px;
}
...
.list{border:1px solid #eef2f7; border-radius:14px; overflow:hidden;}
.listHead, .row{
  display:grid;
...
.row.disabledPick{opacity:.95;}
.c1{display:flex; align-items:center; justify-content:center;}
.nm{font-weight:950; color:#111827; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;}
.id{font-size:10px;}
...
.tag{
  ...
  border:1px solid #e5e7eb;
  background:#fafafa;
  font-weight:950; font-size:10px; color:#374151;
}

.miniActions{display:flex; gap:8px; justify-content:flex-end;}
.foot{
