...
async function generalizeTo(variantIds: string[]) {
  ...
  try {
    ...
    openInfo("Généralisé", "La section Transport a été généralisée sur les variantes ciblées.");
  } catch (e: any) {
...
async function onApplyGeneralize(payload: { mode: "ALL" | "SELECT"; variantIds: string[] }) {
  ...
  const msg =
    payload.mode === "ALL"
      ? "Confirmer la généralisation de la section Transport sur TOUTES les variantes ?"
      : `Confirmer la généralisation de la section Transport sur ${ids.length} variante(s) ?`;

  openConfirm("Généraliser Transport", msg, async () => {
  ...
  <div class="page">
    <div class="top">
      ...
      <div class="actions">
        ...
        <button class="btn" type="button" :disabled="!variant || genBusy || saving" @click="genOpen = true">
          {{ genBusy ? "..." : "Généraliser" }}
        </button>
