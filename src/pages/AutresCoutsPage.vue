...
/* =========================
   ...
   -> calculés depuis les formules (comme dans tes pages)
========================= */
...
type DraftRow = {
  ...
  label: string;
  unite: Unite;
  valeur: number; // selon unité: MAD, MAD/mois, MAD/m3, % CA
};

const rows = ref<DraftRow[]>([]); // ✅ FIX: ref => pas d'undefined
...
function totalFor(r: DraftRow): number {
  ...
  if (r.unite === "MOIS") {
    return v * clamp(dureeMois.value);
  }

  if (r.unite === "M3") {
    return v * clamp(volumeTotal.value);
  }
...
function monthlyFor(r: DraftRow): number {
  // Convertit en MAD/mois si possible
  const v = clamp(r.valeur);
...
const totalGlobal = computed(() => {
  return rows.value.reduce((s, r) => s + totalFor(r), 0);
});
      ...
      <template v-else>
        ...
        <div class="kpis">
          <div class="kpi">
            ...
            <div class="kVal">{{ n(perM3Global, 2) }} <span>DH/m³</span></div>
          </div>
          ...
          <div class="kpi">
            <div class="kLbl">%</div>
            <div class="kVal">{{ n(pctGlobal, 2) }} <span>%</span></div>
          </div>
        </div>
        ...
        <div class="panel">
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(r, idx) in rows" :key="r._id">
                  <td>
                    <input class="input" v-model="r.label" placeholder="Ex: Sécurité, Gardiennage..." />
                  </td>
                  ...
                  <td class="r mono">{{ n(perM3For(r), 2) }}</td>
                  <td class="r mono"><b>{{ money(totalFor(r), 2) }}</b></td>
                  <td class="r mono">{{ n(pctFor(r), 2) }}%</td>
                ...
                <tr v-if="rows.length === 0">
                  <td colspan="6" class="muted pad">Aucun autre coût.</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="footNote muted">
            Note : les unités <b>M3</b> et <b>%CA</b> sont calculées sur le volume total et le CA total de la variante.
          </div>
        </div>
      </template>
    </template>
